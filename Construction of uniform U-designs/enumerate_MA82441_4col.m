function enumerate_MA82441_4col(id,flag,outfilename)
...
N = size(D0,1);
if n~=4 || id(end)~=5
    error('The length of id must be 4 and the last must be 5!\n');
end
    ...
    for a2 = 1:Alen
        ...
        for b1 = 1:Alen
            for b2 = 1:Alen
                k = 2;
                i1 = D(:,k)==0; i2 = D(:,k)==1;
                L(i1,k) = A(:,b1); L(i2,k) = A(:,b2)+4;
                for c1 = 1:Alen
                    for c2 = 1:Alen
                        k = 3;
                        i1 = D(:,k)==0; i2 = D(:,k)==1;
                        L(i1,k) = A(:,c1); L(i2,k) = A(:,c2)+4;
                        for d1 = 1:Alen
                            for d2 = 1:Blen
                                k = 4;
                                [~,i4] = sort(D(:,end));
