function [Disc0, Disc_vec, L0] = TA_CD_LHD_1(groups,D,q,OutIter,InIter,T0,T1,Reps,writedata)
% 20131001 
% 基于 TA 算法求解均匀 LHD
...
for rep = 1:Reps
    ...
    Disc=(13/12)^n+2*Disc+sum(diag(sigma))-sum(sigma1);
...
OutIter = 500*InIter;
Reps = 1;
 [Disc0, Disc_vec, L0] = TA_CD_LHD(D0,q,OutIter,InIter,1e-3,1e-6,1,0);
 fprintf('%.8f\n',Disc0);
%}
