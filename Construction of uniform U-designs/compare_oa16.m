...
dims = [2,3];
N = 16;
outfile = fopen('MA LHD 20151016/comp_oa16_20151016.txt','w');
for n = 4:8
    %Mm0 = zeros(Reps,1);
    %MProj0 = zeros(Reps,1);
    CD0 = zeros(Reps,1);
    ...
    Mmp0 = zeros(Reps,2);
    for rep = 1:Reps
        Lname = strcat('MA LHD 20151016/L0_',int2str(N),'_',int2str(n),'_',int2str(rep));
        L = importdata(Lname);
        N = size(L,1);
    ...
    Mmp2 = zeros(Reps,2);
    for rep = 1:Reps
        Lname = strcat('MA LHD 20151016/L2_',int2str(N),'_',int2str(n),'_',int2str(rep));
        L = importdata(Lname);
        N = size(L,1);
