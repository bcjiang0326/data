...
fprintf('CD:\n');
outfile = fopen('MA LHD 20150126/result_20150703.txt','w');
fprintf(outfile,'oa(36,2^n3,2)\n');
fprintf(outfile,'  &   &     MA-based LHD    &  non-MA-based LHD  &  TA\\\\ \n');
fprintf(outfile,'N & n & ACD & time & ACD  & time & ACD  & time \\\\ \n');
...
for n = 2:6
    q_oa = 2*ones(n+1,1); q_oa(end) = 3;
    q_lh = N*ones(n,1);
    
    % 1.计算 uniform MEA-based LHD
    D = OA(:,[v(1:n),v(end)]);
    groups = zeros(size(D));
    for j = 1:n
        [~,groups(:,j)] = sort(D(:,j));
