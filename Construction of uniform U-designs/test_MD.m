...
for lop = 1:length(name)
    ...
    for n = 3:2:9
        ...
        t0 = cputime;
        [Disc0, Disc_vec0, L0] = TA_MD_LHD(D,q_oa,OutIter,InIter,T0,T1,Reps,0);
        t0 = cputime-t0;
        % 写入到文件中
        ...
        for rep = 1:Reps
            ...
            [Disc_vec1(rep),~, L] = TA_MD_LHD(OA(:,id),q_oa,OutIter,InIter,T0,T1,1,0);
        ...
        t2 = cputime;
        [Disc2, Disc_vec2, L2] = TA_MD(N,q_lh,OutIter,InIter,T0,T1,Reps,0);
        t2 = cputime-t2;
