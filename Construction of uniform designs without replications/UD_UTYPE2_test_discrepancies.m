function [x wd2 info] = UD_UTYPE2_test_discrepancies(Fun,q0,beq,x0,upperbnd,lowerbnd,opt0)
% 2012/5/7  在原有版本的叶节点处进行了修改。 当 beq==2 或者 m-bgn+1-beq==2时均
% 视为叶节点，并采用新算法。添加 info(8)
% 2012/4/24 2012/4/25 
...
%           opt0(2):是否采用 wd2Ls 中的 neighborhood 技术 进行底层运算。是 = 1，default = 1
...
if opt(2) % 叶节点加速技术 2012/5/10，
    % 该技术重点处理 1 的个数减少至 2 或者 0 的个数减少至 2 情形，
    % 对于初始 n == 1 或者 n == m-1 情形不在本程序考虑范围之内。
    % 因为 n == 1 则任意可行解均为最优解，n == m-1，采用 wd2Ls即可，即
    % 做一次循环
    
    
    if beq == 2
        info1(7) = info1(7) + 1;
        delta = H(m-1,m) +f(m-bgn+1)+f(m-bgn); 
        k1 = m-1; k2 = m; % k1,k2 记录最优的取 1 位置
        % i,j处取1，对所有可能做循环
        for i = bgn:m-2
            delta_i = H(i,i+1) + f(i+1-bgn+1); % j = i+1 情形
            kj = i+1; %记录 i 取 1 时，最优的 j. 
            for j = i+2:m
                delta_i_ = H(i,j) + f(j-bgn+1);
                if delta_i > delta_i_ + epsilon
                    delta_i = delta_i_;
                    kj = j;
                end
            end
            delta_i = delta_i + f(i-bgn+1);
            if delta > delta_i + epsilon
                delta = delta_i;
                k1 = i;
                k2 = kj;
            end
        ...
        return;
    elseif m-bgn+1-beq == 2
        info1(7) = info1(7) + 1;
        delta = -f(m-bgn)-f(m-bgn+1)-sum(H(m-1,bgn:m-2))-sum(H(m,bgn:m-1));
        k1 = m-1; k2 = m;
        for i = bgn:m-2
            % delta_i 初始时对应 i,j = i+1.注意 H(i,j)只能减去一次
            delta_i = -f(i+1-bgn+1)-sum(H(i+1,[bgn:i-1,i+2:m]));
            kj = i+1;
            for j = i+2:m
                delta_i_ = - f(j-bgn+1)-sum(H(j,[bgn:i-1,i+1:j-1,j+1:m]));
                if delta_i > delta_i_ + epsilon
                    delta_i = delta_i_;
                    kj = j;
                end
            end
            delta_i = delta_i -f(i-bgn+1)-sum(H(i,[bgn:i-1,i+1:m]));
            if delta > delta_i + epsilon
                delta = delta_i;
                k1 = i;
                k2 = kj;
        ...
        v = [bgn:k1-1,k1+1:k2-1,k2+1:m];
        fval = 0.5*sum(sum(H(v,v)))+sum(f(v-bgn+1));
        if fval < upperbnd - epsilon
            % 此处是第一处可能更新 upperbnd 的位置
            upperbnd = fval;
            xx(bgn:end) = 1;
    ...
    elseif beq == 1
        ...
        return;             
    elseif  m-bgn+1-beq == 1 
        info1(7) = info1(7)+1;
        delta = -sum(H(m,bgn:m-1))-f(end);
        k = m;%表示第 m 个元素为 0.
        for i = bgn:m-1
            delta_i = -sum(H(i,[bgn:i-1,i+1:m]))-f(i-bgn+1);
            if delta > delta_i + epsilon
                delta = delta_i;
                k = i;
            end
