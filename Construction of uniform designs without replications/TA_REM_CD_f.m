function [Disc0, Disc_vec, ID0, compDisc_vec,D0] = TA_REM_CD_f(n,q,OutIter,InIter,T0,T1,Reps,writedata)
% 20121203 
...
%       compDisc_vec: 记录每次重复中最优设计的补设计的 Disc
%       D0: 得到的最优矩阵
s = size(q,2);
if s~=1
    error('TA_REM_CD: Input variable q must be a s-by-1 vector!\n');
end
if nargin < 7
    Reps = 1;
    writedata = 0;
...
dd = ones(s,1);
for i=s-1:-1:1
    dd(i)=q(i+1)*dd(i+1); %prod(q(i+1:end));
end
epsilon = 1e-11;
N = dd(1)*q(1);
...
for rep = 1:Reps
    ...
    Disc_vec(rep) = Disc;
    if nargout > 3
        %compDisc_vec(rep) = CD2_From_comp(q,ID);
        ID_rep = ID;
    end
    ...
    if nargout >3
        compDisc_vec(rep) = CD2_From_comp(q,ID_rep);
    end
end
