function [x wd2 info] = UD_UTYPE_NTree(Fun,q0,beq,x0,upperbnd,lowerbnd,opt0)
% 2012/4/24 2012/4/25 2012/4/26
% 2012/4/26 将此算法改为不定叉树检索，比二叉树检索略快
% 剪枝技术：1.Convex optimization；2.t；3.U-type design
% 顺序分裂，因此在递归过程中不需要vec变量。取而代之的是全局变量j
% Input
...
if nargin > 3 && ~isempty(x0) && ( sum(x0==1) ~= beq || sum(x0==1) + sum(x0==0)~= m )
...
if nargin <= 6 || isempty(opt) || opt(3) < 0 || opt(3) > 1
    opt = zeros(3,1);
    opt(3) = 1;
end
...
% 输入变量不同情况处理
if nargin < 5 || isempty(upperbnd)
    upperbnd = inf;
end
info1(4) = upperbnd;
if nargin < 6 || isempty(lowerbnd)
    lowerbnd = -inf;
end
            ...
            for l = 1:(m-bgn+1-beq) % 此时 length(v0) == m-bgn+1-beq    
                if length(v1) ~= 1    
                    temp_delta = -( sum(H(v1(k),v1)) - 0.5*H(v1(k),v1(k)) ) + ...
                            ( sum(H(v0(l),[v1(1:k-1);v1(k+1:end);v0(l)])) - 0.5*H(v0(l),v0(l)) );        
                else
...
while true    
    ...
    % 2012/4/26 此块改进原有 0 分支算法, 变为多叉树
    % 程序运行至此处，说明分裂位置 splt 取 1 分支已经做完，此时应判断 splt 是否
    % 可取 0.并从 splt+1 位置处开始寻找新的可以取 1 的第一个分裂位置
    % 因此，本程序块寻找当 splt 取 0 时下一个可以分裂的位置，即在 splt 取 0 条件下，
    % 下一个可以取 1 的位置。程序块：
    [Judge0 Rule0] = JudgeByU0(LelCom); % 此时的 LelCom 对应的是 splt
    if ~Judge0
        % 若不可以取 0 分支，回退 NGetRestr0, NoTriesPerLel 并返回
        if splt > bgn
            NoTriesPerLel = NoTriesPerLel-NTPL;
            NGetRestr0 = NGetRestr0 - NGR0;
        end
    ...
    % 程序运行至此，说明可以取 0 分支，则应取 0 分支，更新 NGetRestr0,
    % NoTriesPerLel. 并将搜索起始设为 splt+1
    NoTriesPerLel(LelCom) = NoTriesPerLel(LelCom)+1;
    NTPL(LelCom) = NTPL(LelCom)+1;
    NGetRestr0(Rule0) = NGetRestr0(Rule0)+1;
    NGR0(Rule0) = NGR0(Rule0)+1;
