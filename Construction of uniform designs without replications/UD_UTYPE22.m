function [x wd2 info] = UD_UTYPE22(q0,beq,x0,upperbnd,lowerbnd,opt0)
...
if opt(2) %此时将 wd2Ls 中的 neighborhood 技术加入到剪枝技术中
    ...
    %if beq == 1 || m-bgn+1-beq == 1 
    %    error('Do not deal with n ==1 or n == m-1 ! Call wd2Ls\n');       
    %end
    if beq == 2
        info1(7) = info1(7) + 1;
        delta = H(m-1,m) +f(m-bgn+1)+f(m-bgn); 
        k1 = m; k2 = m-1; % k1,k2 记录最优的取 1 位置
        % i,j处取1，对所有可能做循环
        for i = bgn:m-2
    ...
    elseif m-bgn+1-beq == 2
        info1(7) = info1(7) + 1;
        delta = -f(m-bgn)-f(m-bgn+1)-sum(H(m-1,bgn:m-2))-sum(H(m,1:m-1));
        k1 = m-1; k2 = m-2;
        for i = bgn:m-2
            % delta_i 初始时对应
